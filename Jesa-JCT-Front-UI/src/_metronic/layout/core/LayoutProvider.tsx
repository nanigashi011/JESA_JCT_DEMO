...
const LayoutProvider: FC<WithChildren> = ({children}) => {
  ...
  useEffect(() => {
    ...
    // Force enable aside sidebar on first load
    const currentConfig = LayoutSetup.config
    if (!currentConfig.aside.display || currentConfig.aside.menuIcon !== 'font') {
      const updatedConfig = {
        ...currentConfig,
        aside: {
          ...currentConfig.aside,
          display: true,
          menuIcon: 'font' as const,
        }
      }
      LayoutSetup.setConfig(updatedConfig)
      LayoutSetup.updatePartialConfig(updatedConfig)
