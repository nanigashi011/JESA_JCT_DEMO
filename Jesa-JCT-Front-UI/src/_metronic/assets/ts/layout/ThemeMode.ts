...
type Mode = "light" | "dark" | "dark_blue" | "dark_gradient" | "system";
...
class ThemeMode {
	...
	public setMode = (mode: Mode, menuMode: Mode | ""): void => {
		// Check input values - allow light, dark, and dark variants
		if (mode !== "light" && mode !== "dark" && mode !== "dark_blue" && mode !== "dark_gradient") {
			return;
		}
	...
	public flipImages = () => {
		...
		const isDarkMode = currentMode === "dark" || currentMode === "dark_blue" || currentMode === "dark_gradient";
		
		document
			.querySelectorAll<HTMLElement>("[data-kt-img-dark]")
			?.forEach((item: HTMLElement) => {
				if (item.tagName === "IMG") {
					if (isDarkMode && item.hasAttribute("data-kt-img-dark")) {
						item.setAttribute(
							"data-kt-img-light",
